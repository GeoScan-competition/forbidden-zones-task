...
Ваша задача — разработать алгоритм, обеспечивающий автономный полёт дрона от старта до финиша как можно быстрее при обходе запретных зон.
* Запретные зоны задаются в текстовом файле (см. формат ниже).
* Скорость полёта ограничена 1 м/с.
...
> Когда счёт достигнет нуля, попытка прекращается, а в результат записывается *DNF (Did Not Finish — не финишировал)*.
...
> [!NOTE]
> Оценка может проводиться **на одной или нескольких конфигурациях зон**.
> Решение участника должно быть **универсальным** и корректно работать в любых предложенных условиях.
> В зачёт идёт **сумма баллов**, набранных на всех конфигурациях.

## 🗺 Как открыть карту с зонами
Чтобы увидеть карту с зонами в симуляторе:
1. **Запустите лаунчер** и нажмите кнопку **«Запустить симулятор»**.
2. В появившемся окне **ещё раз нажмите «Запустить симулятор»** — это откроет сам симулятор.
3. В меню симулятора выберите сценарий **«Арена полигон»**.
4. После загрузки нажмите клавишу **`C`**, чтобы переключить камеру на свободную.
5. В правом верхнем углу нажмите кнопку **«+»** (раскрытие панели).
6. Нажмите иконку **кубика с курсором** — это раздел *«Интерактивные объекты»*.
7. В списке слева выберите пункт **«Запретная зона»**.

Теперь на карте отобразятся все зоны, описанные в вашем `zones.txt`.
Если вы измените файл и перезапустите симулятор, зоны автоматически обновятся.
...
5. **Запуск вашего решения с этим файлом**
   ...
   ```
   В Windows путь можно заключать в кавычки `"C:\.....\GeoscanSimulator\zones.txt"`.
...
## 🖥 Симулятор
Лаунчер и ключ активации доступны в тестирующей системе.
...
Если симулятор не обнаружит видеодрайвер в ОС Windows, то после запуска программы отобразится окно, в котором будет предложено перейти на сайт производителя видеочипа и скачать поддерживаемый видеодрайвер.

⚠️ Если компьютер не соответствует указанным характеристикам или не установлен актуальный драйвер видеокарты, стабильная работа симулятора не гарантируется.
